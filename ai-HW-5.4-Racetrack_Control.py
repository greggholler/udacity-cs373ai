...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        x = self.x
        y = self.y
        o = self.orientation % (2 * pi)

        r = radius

        # top straight-away (r <= x <= 3r) and orientation to the right
        #if((x >= r) and (x <= (3 * r)) and ((o <= 0.5) or (o >= 1.5))):
        if((x >= r) and (x <= (3 * r)) and ((o <= (pi * 0.5)) or (o >= (pi * 1.5)))):
            cte = (y - (2 * r))
        # bottom straight-away (r <= x <= 3r) and orientation to the left
        elif ((x >= r) and (x <= (3 * r)) and ((o > (pi * 0.5)) or (o < (pi * 1.5)))):
            cte = - y
        # in the left half-circle
        elif (x < r):
            cte = (sqrt(((r-x)**2) + ((y-r)**2))) - r
