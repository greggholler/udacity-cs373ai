...
def plan(warehouse, dropzone, todo):
    ...
    for i in range(len(todo)):
        ...
        # Calculate the heuristic for the current box location
        heuristic = calc_heuristic(warehouse, box_location, delta)
        
        # Run A*
        cost += run_Astar(warehouse, dropzone, box_location, heuristic, delta)
...
def calc_heuristic(map, target, move_cost):
    # Calculate the heuristic
    #     map - matrix containing the map. 0 is a free space
    #     target - x,y coordinate of target
    #     move_cost - list of available moves. Each element contains the x,y,cost 
    heuristic = [[99 for col in range(len(map[0]))] for row in range(len(map))]
    change = True
    while(change):
        change = False
        for x in range(len(map)):
            for y in range(len(map[0])):
                ...
                if((target[0] == x) and (target[1] == y)):
                    ...
                    for a in range(len(move_cost)):
                        x2 = x + move_cost[a][0]
                        y2 = y + move_cost[a][1]

                        if((x2 >= 0) and (x2 < len(map)) and (y2 >= 0) and (y2 < len(map[0]))):
                            if(heuristic[x][y] > (heuristic[x2][y2] + move_cost[a][2])):
                                heuristic[x][y] = heuristic[x2][y2] + move_cost[a][2]
                                change = True

    return heuristic            

def run_Astar(map, start_cell, target, heuristic, delta):

    expand = [[-1 for col in range(len(map[0]))] for row in range(len(map))]
    closed = [[0 for col in range(len(map[0]))] for row in range(len(map))]
    closed[start_cell[0]][start_cell[1]] = 1
    ...
    while not found and not resign:
        ...
        if len(open) == 0:
            ...
            if((x == target[0]) and (y == target[1])):
                found = True
                map[x][y] = 0
                current_cost = 2 * count_path_cost(search_set[s])
            else:
                for i in range(len(delta)):
                    x2 = x + delta[i][0]
                    y2 = y + delta[i][1]
                    if((x2 >= 0) and (x2 < len(map)) and (y2 >= 0) and (y2 < len(map[0]))):
                        if(((closed[x2][y2] == 0) and (map[x2][y2] == 0)) or
                           ((x2 == target[0]) and (y2 == target[1]))):
