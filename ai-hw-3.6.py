...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if abs(turning_angle) < 0.001:
            ...
            x = cx + (sin(self.orientation + turning_angle) * turning_radius)
            y = cy - (cos(self.orientation + turning_angle) * turning_radius)
            theta = (self.orientation + turning_angle) % (2 * pi)
    ...
    # according to
    #           self.bearing_noise
    def sense(self, include_bearing_noise = 1): 
        Z = []

        for i in range(len(landmarks)):
            dy = landmarks[i][0] - self.y
            dx = landmarks[i][1] - self.x
            val = atan2(dy,dx) - self.orientation
            if(include_bearing_noise):
                val *= self.bearing_noise
