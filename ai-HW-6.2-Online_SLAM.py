...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    #
    # Matrix dimension will be one pose and all landmarks multiplied by 2 (x,y)
    dim = 2 * (num_landmarks + 1)
    landmark_index = 2 
 
    # building expand/take lists
    # sebastian did it better using list = [0,1] + range(4,dim+2)           
    expand_list = [0,1]
    take_list = [2,3]
    for i in range(num_landmarks * 2):
        expand_list.append(i+4)
        take_list.append(i+4)
    ...
    for i in range(len(data)):
        ...
        for j in range(len(Z)):
            ...
            # x and y will always be in the first two indexes
            x = 0
            y = 1
            landmark_x_index = 2 + (2 * landmark_number)
            landmark_y_index = landmark_x_index + 1
        ...
        # Add row/col for new path data
        omega = omega.expand(omega.dimx + 2, omega.dimy + 2, expand_list)
        xi = xi.expand(xi.dimx + 2, 1, expand_list, [0])

        x_t0 = 0
        y_t0 = 1
        x_t1 = 2
        y_t1 = 3
        omega.value[x_t0][x_t0]     += val
        omega.value[x_t0][x_t1]   -= val
        omega.value[x_t1][x_t0]   -= val
        omega.value[x_t1][x_t1] += val
        omega.value[y_t0][y_t0]     += val
        omega.value[y_t0][y_t1]   -= val
        omega.value[y_t1][y_t0]   -= val
        omega.value[y_t1][y_t1] += val
        ...
        omega1 = omega.take(take_list, take_list)
        xi1 = xi.take(take_list,[0])

        ########################################
        # Fold A, B, and C into surviving matrix
